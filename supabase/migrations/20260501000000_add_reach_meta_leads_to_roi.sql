-- RPC get_ad_roi_adset_daily に reach・meta_leads_count を追加
-- 戻り値の型変更のため DROP → CREATE が必要
DROP FUNCTION IF EXISTS public.get_ad_roi_adset_daily(uuid, date, date, boolean);

-- leads_count は CRM leads テーブル（FM未同期のため常に0）→ ad_spend_daily.leads_count を使用
...
AS $$
  SELECT
    ...
    coalesce(d.reach, 0)::bigint AS reach,
    -- Meta API から同期した広告リード数を使用（FM連携前のPhase1）
    coalesce(d.leads_count, 0)::bigint AS leads_count,
    (
