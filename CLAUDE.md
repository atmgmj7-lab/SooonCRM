...
## 項目削除の禁止
- ユーザーから「削除して」「消して」「不要」と明示的に指示された項目のみ削除する
- 実装上の都合・リファクタリング・整理目的での無断削除は全て禁止
- 「使われていない」「古い」と判断しても削除しない。コメントアウトも禁止
- UIコンポーネントのフィールド・セクション・カラムは特に厳守

## 修正時のルール
- 既存機能を壊さない
- 追加・修正のみを行い、既存コードへの影響を最小限にする
